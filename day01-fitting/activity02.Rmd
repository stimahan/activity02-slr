...
1.  What are the dimensions of the dataset? What does each row represent?
    1458 rows and 123 variables/columns. The rows show human freedom data for different countries from 2008-2016.
...
hfi_2016 |>
  ggplot(aes(x = countries, y = pf_score)) +
  geom_bar(stat = "identity")
```

```{r distribution-plots}
hfi_2016 |>
  ggplot(aes(x = countries, y = pf_expression_control)) +
  geom_bar(stat = "identity")
```
...
The relationship looks relatively linear. I would feel pretty comfortable using a linear model to predict the personal freedom score.
...
$\widehat{\texttt{pf\\_score}} = 4.28 + 0.54 \times \texttt{pf\\_expression\\_control}$
...
8.  What does this value mean in the context of this model? Think about what would a "good" value of $R^2$ would be? Can/should this value be "perfect"?
  The regression line explains about 71.4% of the variation in the y-value. If the r^2 value is one, it means that all of the predicted values are identical to the observed values, which would be ideal but not always likely.
